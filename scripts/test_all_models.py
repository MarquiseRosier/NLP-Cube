...
if __name__ == "__main__":
    if len(sys.argv)!=2:
        print("Usage: python3 test_all_models.py path-to-UD-repository-root")
    ...
    for online_model in online_models:
        ...
        disk_path = os.path.join(str(Path.home()), ".nlpcube/models")
        metadata = ModelMetadata()        
        metadata.read(os.path.join(disk_path,model+"-"+str(version),"metadata.json"))
        ...
        cube.load(model, version, tokenization=True, compound_word_expanding=True, tagging=True,
             lemmatization=True, parsing=True) # all enabled, including compound
        start = time.time()        
